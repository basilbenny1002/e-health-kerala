    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative font-sans">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <h2 className="mt-6 text-center text-3xl font-extrabold text-slate-900 tracking-tight capitalize">
          {role.toLowerCase()} {t('portal')}
        </h2>
        <p className="mt-2 text-center text-sm text-slate-600">
          {mode === 'LOGIN' ? t('signInAccess') : t('registerJoin')}
        </p>
      </div>
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-xl sm:px-10 border border-slate-100">
          ...
          {step === 'REQUEST' ? (
            <form className="space-y-6" onSubmit={requestOTP}>
              {mode === 'REGISTER' && (
                <div>
                  ...
                  <div className="mt-1">
                    <input type="text" required onChange={(e) => setName(e.target.value)} placeholder={t('enterName')} className="appearance-none block w-full px-3 py-2.5 border border-slate-300 rounded-lg shadow-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-colors" />
                  </div>
                </div>
              )}
              ...
              {mode === 'REGISTER' && role === 'DOCTOR' && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-slate-700">{t('specialization')}</label>
                    <div className="mt-1">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700">{t('assocHospital')}</label>
                    <div className="mt-1">
                      <select onChange={(e) => setHospitalId(e.target.value)} className="appearance-none block w-full px-3 py-2.5 border border-slate-300 rounded-lg shadow-sm text-slate-900 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white">
                        <option value="">{t('indPractice')}</option>
                        {hospitals.map(h => (
            ...
            <form className="space-y-6" onSubmit={verifyOTP}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700">{t('verifyCodeTitle')}</label>
                <div className="mt-1">
              ...
              <div className="text-center">
                <button type="button" onClick={() => setStep('REQUEST')} className="text-sm text-blue-600 hover:text-blue-500">{t('backToEmail')}</button>
              </div>
            </form>
          )}
