    ...
    <section className="py-24 bg-slate-50 border-t border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-extrabold text-slate-900 mb-4">Find Facilities Near You</h2>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Discover highly-rated hospitals and clinics in your area powered by Google Maps.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-12">
          <form onSubmit={handleSearch} className="flex items-center bg-white p-2 rounded-2xl shadow-sm border border-slate-200">
            ...
            <input
              type="text"
              placeholder="Enter your city or area (e.g. Kochi, Ernakulam)"
              value={location}
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition flex items-center gap-2 disabled:opacity-70"
            >
              {loading ? 'Searching...' : (
                <>
        ...
        {!loading && searched && hospitals.length === 0 && !error && (
          <div className="text-center text-slate-500 py-12">
            No hospitals found in this area. Try a different location.
          </div>
        )}
